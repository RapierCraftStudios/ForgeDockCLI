...
if (!isSupported(process.versions.node)) {
	...
	if (replacement && process.env.FORGEDOCK_NODE_REEXEC !== "1") {
		...
		console.error(
			`ForgeDock requires Node >=${REQUIRED.join(".")}; current Node is ${process.versions.node}.`,
		);
		process.exitCode = 1;
	}
} else {
	...
	const extensionEntry = resolve(
		...
		"dist",
		"tui",
		"forgedock-extension.js",
	);
	const workerAgentModule = resolve(
		packageRoot,
		"dist",
		"tui",
		"worker-agent.js",
	...
	const environmentModule = resolve(
		packageRoot,
		"dist",
		"runtime",
		"controller-environment.js",
	);
	const workerAgentTemplate = resolve(
		packageRoot,
		"agents",
		"forgedock-issue-worker.md",
	);
	const reviewerAgentTemplate = resolve(
		packageRoot,
		"agents",
		"forgedock-reviewer.md",
	);
	if (
		!existsSync(terminalEntry) ||
		!existsSync(extensionEntry) ||
		!existsSync(workerAgentModule) ||
		!existsSync(configModule) ||
		!existsSync(environmentModule) ||
		!existsSync(workerAgentTemplate) ||
		!existsSync(reviewerAgentTemplate) ||
		!existsSync(subagentsEntry)
	) {
		console.error(
			"ForgeDock terminal has not been built. Run `npm run build:pi && npm run build` first.",
		);
		process.exitCode = 1;
	} else {
		...
		if (shouldBootstrapProject(cliArgs)) {
			try {
				const { ensureForgeDockConfig } = await import(
					pathToFileURL(configModule).href
				);
				ensureForgeDockConfig(process.cwd());
			} catch (error) {
				console.error(
					`ForgeDock warning: could not create forge.yaml: ${error instanceof Error ? error.message : String(error)}`,
				);
			}
		}
		refreshConfiguredGitHubApp(packageRoot, cliArgs);
		const { sealVerificationEnvironment } = await import(
			pathToFileURL(environmentModule).href
		);
		...
		for (const name of [
			...
			"USERPROFILE",
			"FORGEDOCK_GIT_BASH",
			"FORGEDOCK_VERIFICATION_PATH",
		]) {
			if (discoveredEnvironment[name])
				process.env[name] = discoveredEnvironment[name];
		}
		const { materializeWorkerAgent } = await import(
			pathToFileURL(workerAgentModule).href
		);
		const workerAgent = materializeWorkerAgent(
			workerAgentTemplate,
			extensionEntry,
...
function refreshConfiguredGitHubApp(packageRoot, args) {
	...
	if (refreshed.status !== 0) {
		...
		console.error(
			`ForgeDock warning: GitHub App credential refresh failed: ${detail}`,
		);
	}
}
	...
	if (process.platform === "win32" && process.env.LOCALAPPDATA) {
		...
		if (existsSync(root)) {
			const versions = readdirSync(root, { withFileTypes: true })
				...
				.map((entry) => ({
					entry,
					match: /^node-v(\d+\.\d+\.\d+)-win-x64$/.exec(entry.name),
				}))
			...
			for (const entry of versions)
				candidates.push(join(root, entry.name, "node.exe"));
		}
		candidates.push(
			join(process.env.LOCALAPPDATA, "pi-node", "current", "node.exe"),
		);
	}
