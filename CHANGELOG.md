...
- Read-only reviewer probes of missing optional paths are treated as negative evidence rather than fatal process errors. A schema-valid reviewer artifact survives a trailing WebSocket/network failure after `structured_output` completed; a genuinely incomplete persisted reviewer is resumed once through the package-owned session lifecycle before any fresh-context replacement. Repeated attempts append a new durable failed Outcome when the cause changes instead of leaving an older failure reason as misleading semantic memory.
