...
- Verification runners snapshot their environment instead of relying on mutable process-global state, preventing concurrent sparse-PATH toolchain probes from contaminating one another. Git Bash and user tools are dynamically discovered before Pi terminal startup, and the complete normalized PATH plus verified Bash path propagate to controllers and verification descendants so nested sparse-PATH checks do not repeat discovery after `npx` has transformed the parent environment. Failure summaries retain bounded TAP assertion context, and baseline-equivalent required failures are rendered as failed rather than misleadingly labeled as passed.
