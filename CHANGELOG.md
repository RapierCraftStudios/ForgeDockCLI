...
- Remediation Build Results now record the complete delivery revision path set relative to an immutable base SHA rather than only the latest uncommitted delta. The base is persisted in durable artifacts and branch metadata so recovery cannot collapse `HEAD..HEAD`; review scope checks distinguish artifact-field references from dotted and extensionless repository paths.
