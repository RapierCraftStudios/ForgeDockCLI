...
- Read-only reviewer probes of missing optional paths are treated as negative evidence rather than fatal process errors, and one operationally failed specialist is retried once in fresh context without discarding successful peer reviews. Repeated attempts now append a new durable failed Outcome when the cause changes instead of leaving an older failure reason as misleading semantic memory.
