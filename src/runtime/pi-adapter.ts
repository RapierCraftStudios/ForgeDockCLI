...
export class PiAgentRuntime implements AgentRuntime {
	...
	async capabilities(): Promise<RuntimeCapabilities> {
		return {
			...
			resumableSessions: Boolean(
				process.env.FORGEDOCK_NESTED_AGENT_URL &&
					process.env.FORGEDOCK_NESTED_AGENT_TOKEN,
			),
	...
	async run<T>(
		...
		const configuredReviewer =
			task.role === "reviewer"
				? splitConfiguredModel(process.env.FORGEDOCK_REVIEWER_MODEL)
				: undefined;
		const provider =
			configuredReviewer?.provider ??
			task.modelPolicy.provider ??
		...
		const thinking =
			task.role === "reviewer"
				? (configuredThinking(process.env.FORGEDOCK_REVIEWER_THINKING) ??
					task.modelPolicy.thinking)
				: task.modelPolicy.thinking;
		if (!provider || !modelId) {
			throw new Error(
				"Pi runtime requires a provider and model (flags, configuration, or PI_PROVIDER/PI_MODEL)",
			);
		}
		if (
			task.role === "reviewer" &&
		...
		const submitTool = defineTool({
			...
			description:
				"Submit the final schema-valid result. This must be your final action.",
			promptSnippet: "Submit the final ForgeDock workflow artifact",
			promptGuidelines: [
				"Call submit_artifact exactly once as your final action.",
			],
			parameters: task.outputSchema,
			async execute(_toolCallId, params) {
				if (submitted !== undefined)
					throw new Error("Artifact was already submitted");
				...
				return {
					content: [
						{
							type: "text" as const,
							text: "ForgeDock accepted the structured artifact.",
						},
					],
					details: params,
					terminate: true,
		...
		const sandboxedTools = await createSandboxedTools(
			task.workspace.cwd,
			task.tools,
		);
		...
		try {
			...
			if (submitted === undefined) {
				throw new Error(
					`Agent ${task.id} ended without calling submit_artifact`,
				);
			}
	...
	async resume<T>(
		...
		if (
			...
			throw new AgentRunError(
				"Pi can resume ForgeDock reviewers only through the persisted nested-agent bridge",
			);
		}
		const configuredReviewer = splitConfiguredModel(
			process.env.FORGEDOCK_REVIEWER_MODEL,
		);
		const provider =
			configuredReviewer?.provider ??
			task.modelPolicy.provider ??
...
async function runNestedReviewer<T>(
	...
	input.emit({
		type: "session.started",
		taskId: task.id,
		sessionRef: provisionalSessionRef,
		provider: input.provider,
		model: input.model,
	});
	const response = await postNestedAgentRequest<{
		output?: T;
		sessionRef?: string;
		provider?: string;
		model?: string;
	...
	if (
		...
		throw new AgentRunError(
			payload.error ??
				`Nested reviewer bridge returned HTTP ${response.status}`,
			{
	...
	if (!Check(task.outputSchema, payload.output)) {
		const details = [...Errors(task.outputSchema, payload.output)]
			.slice(0, 5)
			.map((error) => error.message)
			.join("; ");
		throw new AgentRunError(
			`Nested reviewer returned an invalid structured result: ${details}`,
	...
	return {
		...
		sessionLineage: [
			...new Set([
				...(input.resumeSessionRef ? [input.resumeSessionRef] : []),
				sessionRef,
			]),
		],
...
export function postNestedAgentRequest<T>(input: {
	...
	try {
		...
		throw new Error("Nested reviewer bridge received an invalid URL", {
			cause: error,
		});
	}
	if (target.protocol !== "http:")
		throw new Error(
			`Nested reviewer bridge requires local HTTP, found ${target.protocol}`,
		);
	...
	return new Promise((resolve, reject) => {
		const request = httpRequest(
			...
			(response) => {
				...
				response.once("error", (error) =>
					reject(
						new Error(`Nested reviewer response failed: ${error.message}`, {
							cause: error,
						}),
					),
				);
				response.once("end", () => {
					try {
						...
						reject(
							new Error("Nested reviewer bridge returned invalid JSON", {
								cause: error,
							}),
						);
					}
		...
		request.once("error", (error) =>
			reject(
				new Error(`Nested reviewer transport failed: ${error.message}`, {
					cause: error,
				}),
			),
		);
	...
	const systemPrompt = [
		...
		...(memory.length
			? [
					...
					...memory.map(
						(hit) => `- devdocs/${hit.path} — ${hit.title}: ${hit.summary}`,
					),
				]
			: []),
	].join("\n");
...
function configuredThinking(
	...
	return ["off", "minimal", "low", "medium", "high", "xhigh", "max"].find(
		(level) => level === value,
...
function assertToolPolicy<T>(task: AgentTask<T>): void {
	...
	if (
		task.workspace.mode === "read-only" &&
		(grants.has("edit") || grants.has("write"))
	) {
