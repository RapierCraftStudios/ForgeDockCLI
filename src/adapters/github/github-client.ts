...
const WORKFLOW_LABELS = [
	{
		name: "workflow:investigating",
		color: "1D76DB",
		description: "ForgeDock investigation is active",
	},
	{
		name: "workflow:ready-to-build",
		color: "0E8A16",
		description: "ForgeDock investigation is complete and build is ready",
	},
	{
		name: "workflow:building",
		color: "FBCA04",
		description: "ForgeDock build or verification is active",
	},
	{
		name: "workflow:in-review",
		color: "5319E7",
		description: "ForgeDock pull-request review is active",
	},
	{
		name: "workflow:awaiting-merge",
		color: "D4C5F9",
		description: "ForgeDock review passed and awaits a human merge",
	},
	{
		name: "workflow:merged",
		color: "0E8A16",
		description: "ForgeDock delivery completed and merged",
	},
	{
		name: "workflow:invalid",
		color: "B60205",
		description: "ForgeDock investigation found the issue invalid",
	},
	{
		name: "workflow:decomposed",
		color: "C2E0C6",
		description: "ForgeDock split the issue into smaller delivery units",
	},
	{
		name: "workflow:engine-error",
		color: "B60205",
		description: "ForgeDock runtime or tool failure requires recovery",
	},
	{
		name: "needs-human",
		color: "D93F0B",
		description: "ForgeDock requires a human decision or intervention",
	},
] as const;
...
const REVIEW_FINDING_LABELS = [
	{
		name: "review-finding",
		color: "D93F0B",
		description: "Defect or improvement found during independent PR review",
	},
	{
		name: "needs-validation",
		color: "FBCA04",
		description: "Review finding awaiting validation",
	},
...
export class GitHubClient implements ForgeHost {
	...
	private readonly initializedReviewFindingRepos = new Map<
		string,
		Promise<void>
	>();
	...
	async getRepository(
		...
		if (!parsed.nameWithOwner || !parsed.defaultBranchRef?.name)
			throw new Error("Unable to resolve GitHub repository and default branch");
		return {
	...
	async getIssue(number: number, repo?: string): Promise<GitHubIssue> {
		...
		const result = await this.gh([
			...
			resolvedRepo,
			"--json",
			"number,title,body,url,state,labels,milestone",
		]);
		const issue = JSON.parse(result) as Omit<
			GitHubIssue,
			"repo" | "labels" | "comments" | "milestone"
		> & {
			labels?: Array<{ name?: string }>;
			milestone?: { number: number; title: string } | null;
		};
		const comments = await this.listIssueCommentSnapshots({
			repo: resolvedRepo,
			issue: number,
		});
	...
	async listIssueComments(subject: Subject): Promise<string[]> {
		return (await this.listIssueCommentSnapshots(subject)).map(
			(comment) => comment.body,
		);
	}

	async listIssueCommentSnapshots(
		subject: Subject,
	): Promise<GitHubIssueComment[]> {
		const number = subject.pr ?? subject.issue;
		if (!number)
			throw new Error(
				"GitHub artifacts require an issue or pull request number",
			);
		const result = await this.gh([
			"api",
			`repos/${subject.repo}/issues/${number}/comments?per_page=100`,
			"--paginate",
			"--slurp",
		]);
		const pages = JSON.parse(result) as Array<
			Array<{
				body?: string;
				created_at?: string;
				html_url?: string;
	...
	async postIssueComment(subject: Subject, body: string): Promise<void> {
		...
		await this.gh(
			[
				"api",
				`repos/${subject.repo}/issues/${number}/comments`,
				"--method",
				"POST",
				"--input",
				"-",
	...
	async publishPullRequestComment(input: {
		repo: string;
		pullRequest: number;
		marker: string;
		body: string;
		...
		if (!input.body.includes(input.marker))
			throw new Error(
				"Reviewer comment body is missing its idempotency marker",
			);
	...
	async materializeReviewFinding(input: {
		...
		const legacyMarker = legacyReviewFindingMarker(
			input.repo,
			input.pullRequest.number,
			input.finding,
		);
		const existing = (await this.listAllIssues(input.repo)).find(
			(issue) =>
		...
		const title = boundedGitHubText(
			`fix: ${input.finding.title} (review finding — PR #${input.pullRequest.number})`,
			240,
		).replace(/[\r\n]+/g, " ");
		const affectedFile = reviewFindingPath(input.finding.location);
		const sensitive =
			/security|auth|billing|payment|stripe|credential|secret|token/i.test(
				`${affectedFile ?? ""} ${input.finding.title}`,
			);
		const body = [
			"## Problem",
			"",
			...
			...(input.sourceIssue
				? [`**Delivery issue:** #${input.sourceIssue}`]
				: []),
			...
			...(input.finding.sourceFindingIds?.length
				? [
						`**Source findings:** ${input.finding.sourceFindingIds.map((id) => `\`${boundedGitHubCode(id)}\``).join(", ")}`,
					]
				: []),
			...(input.finding.sourceSessionRefs?.length
				? [
						`**Reviewer sessions:** ${input.finding.sourceSessionRefs.map((ref) => `\`${boundedGitHubCode(ref)}\``).join(", ")}`,
					]
				: []),
			...
			...(input.finding.scopeDisposition
				? [`**Scope disposition:** ${input.finding.scopeDisposition}`]
				: []),
			...(input.finding.scopeRationale
				? [
						`**Scope rationale:** ${boundedGitHubText(input.finding.scopeRationale, 3_000)}`,
					]
				: []),
			...
			"",
			affectedFile
				? `- \`${boundedGitHubCode(affectedFile)}\`${input.finding.location ? ` — ${boundedGitHubText(input.finding.location, 1_000)}` : ""}`
				: "- Location not reported; validate during investigation.",
			"",
			"## Evidence",
			"",
			...
			...(priority === "priority:P3" && !sensitive
				? ["", "<!-- FORGE:BATCHABLE -->"]
				: []),
			"",
			marker,
		...
		const metadata = JSON.parse(
			await this.gh([
				"api",
				`repos/${input.repo}/issues/${input.pullRequest.number}`,
			]),
		) as {
		...
		if (metadata.milestone?.title)
			args.push("--milestone", metadata.milestone.title);
		const url = (await this.gh(args, body)).trim();
		const number = Number(url.split("/").at(-1));
		if (!url || !Number.isSafeInteger(number) || number < 1)
			throw new Error("GitHub did not return a review-finding issue number");
	...
	async reconcileReviewFindings(input: {
		...
		const stale = reviewFindingReconciliationCandidates(
			await this.listAllIssues(input.repo),
			input,
		);
	...
	private async ensureReviewFindingLabels(repo: string): Promise<void> {
		...
		if (!initialization) {
			initialization = Promise.all(
				REVIEW_FINDING_LABELS.map((label) =>
					this.gh([
						"label",
						"create",
						label.name,
			...
			initialization.catch(() =>
				this.initializedReviewFindingRepos.delete(repo),
			);
		}
	...
	async projectRunState(state: RunState): Promise<void> {
		...
		const labelResult = await this.gh([
			"issue",
			"view",
			String(issue),
			"--repo",
		...
		const current =
			(
				JSON.parse(labelResult) as { labels?: Array<{ name?: string }> }
			).labels?.flatMap((label) => (label.name ? [label.name] : [])) ?? [];
		const remove = current.filter(
			(label) =>
				WORKFLOW_LABEL_NAMES.includes(
					label as (typeof WORKFLOW_LABEL_NAMES)[number],
				) && label !== target,
		);
		...
		if ((target && !current.includes(target)) || remove.length)
			await this.gh(args);
	}
	...
	async materializeDecomposition(input: {
		...
		const inheritedLabels = parent.labels.filter(
		...
		const byMarker = new Map(
			existing.flatMap((issue) => {
				const match = /<!-- FORGEDOCK:DECOMPOSITION ([a-f0-9]{64}) -->/.exec(
					issue.body,
				);
		...
		for (const child of ordered) {
			const marker = decompositionMarker(
				input.repo,
				input.parentIssue,
				child.title,
			);
			...
			if (!issue) {
				...
				if (!Number.isSafeInteger(number) || number < 1)
					throw new Error(
						`GitHub did not return a child issue number for '${child.title}'`,
					);
				issue = {
					repo: input.repo,
					number,
		...
		return input.children.map((child) => {
			...
			if (!issue)
				throw new Error(
					`Decomposition child was not materialized: ${child.title}`,
				);
			return issue;
		});
	}
	...
	async getPullRequest(
		...
		const result = await this.gh([
			...
			repo,
			"--json",
			"number,title,body,url,state,headRefOid,headRefName,baseRefName",
		]);
	...
	async getBranchHead(repo: string, branch: string): Promise<string> {
		...
		if (!value.object?.sha)
			throw new Error(`GitHub did not return a head SHA for ${repo}:${branch}`);
	...
	async listBranches(repo: string, prefix: string): Promise<BranchSnapshot[]> {
		...
		>;
		return pages.flat().flatMap((branch) => {
			if (!branch.name?.startsWith(prefix) || !branch.commit?.sha) return [];
			return [{ name: branch.name, headSha: branch.commit.sha }];
		});
	}
	...
	async mergePullRequest(
		...
		if (current.headSha !== expectedHeadSha) {
			throw new Error(
				`Pull request head changed: reviewed ${expectedHeadSha}, current ${current.headSha}`,
			);
		}
		if (current.baseBranch !== expectedBaseBranch) {
			throw new Error(
				`Pull request base changed: expected ${expectedBaseBranch}, current ${current.baseBranch}`,
			);
		}
		await this.gh([
			"pr",
	...
	async materializeBatchIssue(input: BatchIssueInput): Promise<IssueSnapshot> {
		...
		if (!marker)
			throw new Error(
				"Batch issue body is missing its deterministic FORGEDOCK:BATCH marker",
			);
		const existing = (await this.listAllIssues(input.repo)).find(
		...
		await this.gh([
			...
			"C2E0C6",
			"--description",
			"Multiple compatible findings delivered as one verified work unit",
			"--force",
		]);
		const createArgs = [
			"issue",
		...
		const number = Number(url.split("/").at(-1));
		if (!url || !Number.isSafeInteger(number) || number < 1)
			throw new Error("GitHub did not return a batch issue number");
		return {
			repo: input.repo,
			number,
			title: input.title,
		...
		return new Promise((resolve, reject) => {
			...
			child.on("close", (code) => {
				if (code === 0) resolve(stdout);
					...
					reject(
						new Error(
							`gh ${args[0] ?? ""} failed (${code ?? "unknown"}): ${stderr.trim()}`,
						),
					);
			});
...
export function reviewFindingReconciliationCandidates(
	...
	const activeMarkers = new Set(
		input.activeFindings.flatMap((finding) => [
...
export function reviewFindingMarker(
	...
	const identity = /^review-[a-f0-9]{16}$/.test(finding.id)
		? `${legacyIdentity}\n${finding.id}`
		: legacyIdentity;
	return `<!-- FORGEDOCK:REVIEW-FINDING ${createHash("sha256").update(identity).digest("hex")} -->`;
}

function legacyReviewFindingMarker(
	repo: string,
	pullRequest: number,
	finding: ReviewFindingInput,
): string {
	return `<!-- FORGEDOCK:REVIEW-FINDING ${createHash("sha256")
		.update(reviewFindingIdentity(repo, pullRequest, finding))
		.digest("hex")} -->`;
}

function reviewFindingIdentity(
	repo: string,
	pullRequest: number,
	finding: ReviewFindingInput,
): string {
...
function reviewFindingPriority(
	severity: ReviewFindingInput["severity"],
...
function boundedGitHubCode(value: string): string {
	return boundedGitHubText(value, 500)
		.replaceAll("`", "'")
		.replace(/[\r\n]+/g, " ");
}

function decompositionMarker(
	repo: string,
	parentIssue: number,
	title: string,
): string {
	return createHash("sha256")
		.update(
			`${repo.toLowerCase()}#${parentIssue}\n${title.trim().toLowerCase()}`,
		)
		.digest("hex");
}

function orderDecompositionChildren(
	children: DecompositionChild[],
): DecompositionChild[] {
	const byTitle = new Map<string, DecompositionChild>();
	for (const child of children) {
		if (byTitle.has(child.title))
			throw new Error(`Duplicate decomposition child title: ${child.title}`);
	...
	while (remaining.size) {
		const ready = children.filter(
			(child) =>
				remaining.has(child.title) &&
				child.dependsOn
					.filter((dependency) => byTitle.has(dependency))
					.every((dependency) => !remaining.has(dependency)),
		);
		if (!ready.length)
			throw new Error("Decomposition child dependencies contain a cycle");
...
export class GitHubArtifactRepository implements ArtifactRepository {
	constructor(
		readonly client: Pick<
			GitHubClient,
			"listIssueComments" | "postIssueComment"
		>,
	) {}

	async append(artifact: DurableArtifact): Promise<void> {
		const targets: Subject[] =
			artifact.subject.pr && artifact.subject.issue
				? [
						{ repo: artifact.subject.repo, pr: artifact.subject.pr },
						{ repo: artifact.subject.repo, issue: artifact.subject.issue },
					]
				: [artifact.subject];
		for (const target of targets) {
			...
			const exists = comments
				...
				.some((item) => item.id === artifact.id);
			if (!exists)
				await this.client.postIssueComment(
					target,
					renderArtifactComment(artifact),
				);
		}
	}
	...
	async list(
		...
		const targets: Subject[] =
			subject.pr && subject.issue
				? [
						{ repo: subject.repo, pr: subject.pr },
						{ repo: subject.repo, issue: subject.issue },
					]
				: [subject];
		const found = (
			await Promise.all(
				targets.map(async (target) =>
					(await this.client.listIssueComments(target)).flatMap(findArtifacts),
				),
			)
		).flat();
