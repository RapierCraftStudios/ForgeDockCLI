...
import forgedockExtension, {
	executeController,
	FORGEDOCK_READY_STATUS,
	isLifecycleControllerShellCommand,
} from "./forgedock-extension.js";
import {
...
test("commands lazily activate separate semantic native tools without loading Markdown specs", async () => {
	const state = fakePi();
	assert.deepEqual([...state.tools.keys()].sort(), [
		"forgedock_ask_user",
		"forgedock_configure",
		"forgedock_memory_search",
		"forgedock_orchestrate",
		"forgedock_remember",
		"forgedock_resume_orchestration",
		"forgedock_review_pr",
		"forgedock_status",
		"forgedock_tasks",
		"forgedock_work_on",
	...
	await state.commands.get("orchestrate")?.(
		"all open enhancement issues --dry-run",
		commandContext(),
	);
	assert.equal(state.sent.length, 1);
	assert.match(
		state.sent[0]?.content ?? "",
		/resolve it to a concrete eligible issue-number set/,
	);
	assert.match(
		state.sent[0]?.content ?? "",
		/call forgedock_orchestrate exactly once/,
	);
	assert.match(state.sent[0]?.content ?? "", /execution DAG/);
	assert.match(
		state.sent[0]?.content ?? "",
		/Automatic merge .* is the default/,
	);
	assert.doesNotMatch(
		state.sent[0]?.content ?? "",
		/commands\/orchestrate\.md|command spec at/,
	);
	assert.deepEqual(state.active, [
		"read",
		"bash",
...
test("model configuration rejects installed models without available authentication", () => {
	...
	assert.throws(
		() =>
			resolveModelReference("example/luna-5.6", {
		...
		if (name === "subagents:rpc:v1:request" && data.method === "ping") {
			queueMicrotask(() =>
				originalEmit(`subagents:rpc:v1:reply:${data.requestId}`, {
					...
					data: {
						version: 1,
						capabilities: { asyncSpawn: true, fleetStatus: { version: 1 } },
					},
				}),
			);
		}
...
test("human checkpoints use the tabbed decision interview and return typed answers", async () => {
			...
			questions: [
				{
					...
					options: [
						...
						},
						{
							value: "fast",
							label: "Immediate",
							description: "Finishes sooner with more risk",
						},
					],
	...
	assert.deepEqual(
		(result.details as { answers: Record<string, { values: string[] }> })
			.answers.rollout?.values,
		["safe"],
	);
});
...
test("decision interviews normalize stored legacy single-question calls", () => {
	...
	assert.deepEqual(
		normalized.questions[0]?.options.map((option) => option.value),
		["a", "b"],
	);
});
...
test("orchestrate starts only the live DAG ready set without static batch phases", async () => {
		...
		if (name === "subagents:rpc:v1:request" && data.method === "spawn") {
			...
			queueMicrotask(() =>
				originalEmit(`subagents:rpc:v1:reply:${data.requestId}`, {
					...
					data: {
						text: "started",
						details: { asyncId: `test-run-${spawnRequests.length}` },
					},
				}),
			);
		}
	...
	process.env.FORGEDOCK_CONTROLLER_ENTRY =
		"C:/Forge Dock/bin/forgedock-next.mjs";
	try {
				...
				executionPlan: [
					{
						issue: 7,
						title: "Seven",
						summary: "Implement the accepted bounded behavior.",
						priority: 1,
						dependsOn: [],
						claims: ["src/core"],
					...
					},
					{
						issue: 8,
						title: "Eight",
						summary: "Consume Seven's completed behavior.",
						priority: 2,
						dependsOn: [7],
						claims: ["src/api"],
					},
	...
	assert.match(
		spawnRequest.params.model,
		/^openai-codex\/gpt-worker(?::[a-z]+)?$/,
	);
	assert.equal(spawnRequest.params.chain, undefined);
	assert.match(
		spawnRequest.params.task,
		/forgedock_work_on.*\{"issue":7,"dependencies":\[\]/,
	);
	assert.match(spawnRequest.params.task, /"autoMerge":true/);
	assert.match(spawnRequest.params.task, /"resume":true/);
	assert.match(
		spawnRequest.params.task,
		/Implement the accepted bounded behavior/,
	);
	...
	await assert.rejects(
		() =>
			tool.execute(
				...
				undefined,
				undefined,
				{ ...commandContext(), hasUI: false } as any,
			),
...
test("visible DAG delegation dispatches a successor on its predecessor completion event", async () => {
	...
	const run = await delegator.start({
		...
		taskFor: (item) => ({
			...
			task: `Deliver issue #${item.issue}`,
			cwd: process.cwd(),
		}),
		assertCompleted: async (item) => {
			completed.push(item.issue);
		},
...
test("claim-serialized successor dispatches after an invalid predecessor releases its claim", async () => {
	const state = fakePi();
	const launched: number[] = [];
	const originalEmit = state.pi.events.emit.bind(state.pi.events);
	state.pi.events.emit = ((name: string, data: any) => {
		...
		if (name === "subagents:rpc:v1:request" && data.method === "spawn") {
			...
			queueMicrotask(() =>
				originalEmit(`subagents:rpc:v1:reply:${data.requestId}`, {
					...
					requestId: data.requestId,
					success: true,
					data: { details: { asyncId: `claim-run-${issue}` } },
				}),
			);
		}
	}) as typeof state.pi.events.emit;
	...
	const run = await delegator.start({
		items: [
			{
				...
				priority: 1,
				dependencies: [],
				claims: [".github/workflows/ci.yml"],
				labels: [],
				affectedFiles: [],
				memberIssues: [145],
			},
		...
		serializationEdges: [
			{
				predecessor: "issue-145",
				successor: "issue-147",
				overlappingClaims: [".github/workflows/ci.yml"],
			},
		],
		maxParallel: 2,
		taskFor: (item) => ({
		...
		assertCompleted: async (item) => {
			if (item.issue === 145) throw new Error("invalid issue");
		},
		onComplete: (result) =>
			results.push(
				...["issue-145", "issue-147"].map(
					(id) => result.status.get(id) ?? "missing",
				),
			),
	});
	assert.deepEqual(launched, [145]);
	originalEmit("subagent:async-complete", { runId: "claim-run-145" });
	await new Promise((resolve) => setTimeout(resolve, 5));
	assert.deepEqual(launched, [145, 147]);
	originalEmit("subagent:async-complete", { runId: "claim-run-147" });
	await run.completion;
...
test("fresh-rerun authorization cannot be converted back into checkpoint resume", () => {
	assert.deepEqual(
		resolveIssueWorkerRecovery(["needs-human"], false, "rerun"),
		{ rerun: true, resume: false },
	);
	assert.deepEqual(
		resolveIssueWorkerRecovery(["workflow:engine-error"], true, "initial"),
		{ rerun: true, resume: false },
	);
	assert.deepEqual(resolveIssueWorkerRecovery([], false, "resume"), {
		rerun: false,
		resume: true,
	});
});
...
test("visible DAG resume retries failed nodes without replaying completed nodes", async () => {
	...
	const first = await delegator.start({
		...
		onComplete: (result) =>
			results.push(result.status.get("issue-6") ?? "missing"),
	});
...
test("visible DAG recovery applies an explicitly authorized fresh rerun to the failed issue", async () => {
	...
	await assert.rejects(
		() => delegator.resume(first.id, { rerunIssueNumbers: [99] }),
		/already complete|does not match/,
	);
	await delegator.shutdown();
});
...
test("controller subprocess does not inherit the invoking worker role", async () => {
	...
	try {
		const result = await executeController(
			...
			[
				"-e",
				"process.stdout.write(process.env.PI_SUBAGENT_CHILD_AGENT ?? 'clean')",
			],
			process.cwd(),
			undefined,
			() => undefined,
...
test("work-on rejects contradictory fresh-rerun and checkpoint-resume policies", async () => {
	...
	await assert.rejects(
		tool.execute(
			...
			{ issue: 6, rerun: true, resume: true },
			undefined,
			undefined,
			ctx,
		),
...
test("direct work-on defaults to a native non-blocking controller task", async () => {
	...
	writeFileSync(
		entry,
		"setTimeout(() => console.log('controller done'), 50);\n",
	);
	const state = fakePi();
	...
	try {
		...
		for (let attempt = 0; attempt < 100; attempt++) {
			const listed = await tasks.execute(
				"list",
				{ action: "list" },
				undefined,
				undefined,
			...
			const records = (
				listed.details as { records: Array<{ id: string; status: string }> }
			).records;
			if (
				records.find((record) => record.id === details.taskId)?.status ===
				"completed"
			)
				break;
	...
	try {
		const result = handler({
			...
			content: [
				{
					type: "text",
					text: "ENOENT: no such file or directory, access 'workflows/missing.yml'",
				},
			],
		});
		assert.deepEqual(result, {
			...
			content: [
				{
					type: "text",
					text: "File does not exist at the requested path. Treat absence as review evidence and continue with ls/find rather than failing the review.",
				},
			],
		});
		assert.equal(
...
test("shell fallback cannot impose a wall-clock timeout on lifecycle controllers", () => {
	...
	const blocked = guard({
		toolName: "bash",
		input: { command: "node dist/cli/main.js work-on 6 --rerun" },
	});
	assert.deepEqual(blocked, {
		block: true,
		reason:
			"ForgeDock lifecycle controllers cannot be launched through the shell tool or bounded by its wall-clock timeout. Use the active semantic workflow, resume, task-status, or cancellation tool instead.",
	});
	assert.equal(
		guard({
			toolName: "bash",
			input: { command: "node dist/cli/main.js status --issue 6" },
		}),
		undefined,
	);
	assert.equal(
		guard({ toolName: "bash", input: { command: "npm test" } }),
		undefined,
	);
	assert.equal(
		isLifecycleControllerShellCommand("forgedock-next orchestrate 6,7"),
		true,
	);
	assert.equal(
		isLifecycleControllerShellCommand("npm run next -- work-on 6 --rerun"),
		true,
	);
});

test("native command prompts preserve natural-language intent", () => {
	const prompt = buildNativeCommandPrompt(
		"orchestrate",
		"all open issues except blocked",
	);
	assert.match(prompt, /\/orchestrate all open issues except blocked/);
	assert.match(prompt, /concrete eligible issue-number set/);
	assert.match(
		prompt,
		/Never invoke forgedock-next, dist\/cli\/main\.js, or another lifecycle controller through bash\/shell/,
	);
	assert.match(
		buildNativeCommandPrompt("work-on", "6 --resume"),
