...
export async function workOn(
  ...
  try {
    ...
    const built = await buildWorkItem({
      run, intent: input.intent, investigation: investigated.investigation, packet: prepared.packet,
      worktree: workspace.path, ...runtimeOptions,
    }, {
      runtime: dependencies.runtime,
      runs: dependencies.runs,
      ...(dependencies.onAgentEvent !== undefined ? { onAgentEvent: dependencies.onAgentEvent } : {}),
    ...
    run = built.run;
    const commands = input.verification.map((command) => ({ ...command, cwd: workspace?.path ?? input.repoPath }));
    ...
    const published = await publishPullRequest({
      run, intent: input.intent, packet: prepared.packet, buildResult, workspace, baseBranch: input.baseBranch,
    ...
    while (true) {
      const reviewed = await reviewPullRequest({
        run, pullRequest, intent: input.intent, investigation: investigated.investigation,
        packet: prepared.packet, buildResult, workspace: workspace.path,
      ...
      const scopeViolation = blockingFindingOutsidePacket(verdict, prepared.packet);
      if (scopeViolation) {
      ...
      const remediated = await remediateReview({
        run, intent: input.intent, investigation: investigated.investigation, packet: prepared.packet,
        buildResult, verdict, worktree: workspace.path, ...runtimeOptions,
      }, {
      ...
      const revision = await publishRemediationRevision({ run, pullRequest, buildResult, workspace }, {
        git: dependencies.git, host: dependencies.host, runs: dependencies.runs,
      });
      run = revision.run;
