...
describe("lean orchestration scheduler", () => {
	it("honors dependencies, priority, concurrency and path claims", async () => {
		...
		const result = await runSchedule(items, 3, async (item) => {
			if (active.some((other) => claimsConflict(item.claims, other.claims)))
	...
	it("materializes claim conflicts as stable DAG edges instead of static batches", () => {
		...
		assert.deepEqual(
			materialized.edges.map((edge) => [edge.predecessor, edge.successor]),
			[["a", "b"]],
		);
		assert.deepEqual(
			materialized.items.find((item) => item.id === "b")?.dependencies,
			[],
		);
		...
		assert.deepEqual(
			preview.initialReady.map((item) => item.id),
			["c", "a"],
		);
	...
	it("dispatches a claim-serialized successor after an invalid predecessor terminates", async () => {
		...
		const schedule = materializeClaimDependencies(items);
		const run = runSchedule(
			...
			async (item) => {
				...
				if (item.id === "invalid") throw new Error("invalid issue");
			},
			schedule.edges,
		);
		await sleep(5);
		assert.deepEqual(started, ["invalid"]);
		releases.get("invalid")?.();
		await sleep(5);
		assert.deepEqual(started, ["invalid", "successor"]);
		releases.get("successor")?.();
		const result = await run;
		assert.equal(result.status.get("invalid"), "failed");
		assert.equal(result.status.get("successor"), "completed");
	});
	...
	it("blocks dependents when a prerequisite fails", async () => {
		const result = await runSchedule(
			...
			2,
			async (item) => {
				if (item.id === "a") throw new Error("failed");
			},
		);
...
describe("worker leases", () => {
	...
	it("requires the current token for heartbeat", () => {
		...
		assert.throws(
			() => leases.heartbeat("issue-2", "wrong", 100, 1_050),
			/another worker/,
		);
		assert.equal(
			leases.heartbeat("issue-2", lease?.token ?? "", 100, 1_050).expiresAt,
			1_150,
		);
	});
});
