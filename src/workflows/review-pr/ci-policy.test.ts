...
describe("review CI policy", () => {
  ...
  it("owns selected checks and ignores its self marker", () => { const a = assessPullRequestCi(pr("fix/a", "main"), gate([{ name: "build", state: "passed" }, { name: "test", state: "pending" }, { name: "Check for FORGE gate markers", state: "failed" }]), { ...DEFAULT_REVIEW_CI, deliveryChecks: ["build", "test"] }); assert.deepEqual(a.pending.map((x) => x.name), ["test"]); assert.equal(a.failed.length, 0); });
