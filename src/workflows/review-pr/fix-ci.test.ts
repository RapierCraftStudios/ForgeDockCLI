...
describe("review CI auto-fix", () => {
  it("verifies and publishes one exact-head repair commit with readable scoped roots", async () => { const host = new Host(); const workspaces = new Workspaces(host); const runtime = new FakeAgentRuntime([{ summary: "Fixed assertion", diagnosis: "stale expectation", changedPaths: ["src/a.ts"] }]); const result = await makePullRequestCiGreen({ repo: "a/b", pullRequest: 8, policy }, { runtime, host: host as unknown as ForgeHost, workspaces, verifier: { async run() { return [{ command: "test", status: "passed", durationMs: 1 }]; } }, verificationCommands: () => [{ id: "test", command: "npm", args: ["test"], timeoutMs: 1000, required: true }], wait: async () => undefined }); assert.equal(result.attempts, 1); assert.equal(workspaces.pushed, 1); assert.match(runtime.tasks[0]?.instructions ?? "", /untrusted evidence/); assert.deepEqual(runtime.tasks[0]?.workspace.scope.readRoots, ["src"]); assert.deepEqual(runtime.tasks[0]?.workspace.scope.writePaths, ["src/a.ts"]); });
  it("treats a known-empty required-check projection as a no-op instead of inventing a repair", async () => {
    const host = new Host();
    Object.defineProperty(host, "getPullRequestMergeGate", { value: async () => ({
      repo: "a/b", pullRequest: 8, headSha: host.sha, baseBranch: "main", mergeable: true,
      requiredChecksProvenance: "github-none" as const, requiredChecksHeadSha: host.sha,
      requiredChecks: [], observedAt: new Date().toISOString(),
    }) });
    const workspaces = new Workspaces(host);
    const runtime = new FakeAgentRuntime();
    const result = await makePullRequestCiGreen(
      { repo: "a/b", pullRequest: 8, policy: { ...policy, requiredChecksDefault: "if-present" } },
      {
        runtime, host: host as unknown as ForgeHost, workspaces,
        verifier: { async run() { return []; } }, verificationCommands: () => [], wait: async () => undefined,
      },
    );
    assert.equal(result.attempts, 0);
    assert.deepEqual(result.fixes, []);
    assert.equal(workspaces.pushed, 0);
