...
- There is no fixed tool-call quota or runtime read/search cutoff for this read-only role. Continue targeted reads/searches as needed to cover the assigned paths and capabilities; do not broaden into repository-wide exploration. `findings: []` is a complete result when no defect is proven.
