...
**Status**: Execution-ready · **Scope**: the entire ForgeDock Next runtime, end to end · **Owner**: ForgeDock maintainers
...
The last observed Next test run is **229 tests: 228 passing, 1 failing**. The failure is the stale expected grant list in `src/workflows/work-on/build.test.ts`; the implementation correctly includes `compute` for builders. The last recorded legacy result is **1,813 passing, 8 intentionally skipped, 0 failing**. `npm run build` was previously green.
...
- `npm run test:next` reports 229/229 (or the refreshed baseline count with zero failures).
...
| Authority ports | `src/core/ports/forge-host.ts`, `src/adapters/github/github-client.ts`, adapter tests | Idempotent finding/child materialization and issue trajectory comments |
...
Run `npm run test:legacy` when shared adapters, ports, configuration, or legacy-facing package behavior are touched. Run focused tests after each change, then the full suite before merging.
