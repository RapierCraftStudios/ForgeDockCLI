...
A capability is not a free-standing semantic record. It is a controller-issued canonical envelope with exactly the common top-level fields `profile`, `capabilityId`, `delegationId`, `body`, and `proof`; unknown fields are rejected. Its `body` contains exactly the common fields governed by the semantic-body table below (including the registered conditional presence of `runId`) plus the action-specific members registered in this section. Only `artifact.append` carries `artifactAppend`; only `pr.publish` carries `publicationTarget`; revision-member presence remains governed by the closed matrix. This `merge.request` example carries `reviewedSha`:
...
`artifactAppend` is required only for `artifact.append` and forbidden for every other action. It is a closed object with exactly `artifactProfile`, `artifactKind`, `artifactId`, `payloadDigest`, `expectedSequence`, and `predecessor`. `artifactProfile` is exactly `forgedock.protected/v1`; `artifactKind` is exactly one appendable kind registered in §3.1; `artifactId` is the exact new protected artifact ID; `expectedSequence` is a safe integer; and `predecessor` is exactly `null` for sequence zero or the expected prior artifact digest. `payloadDigest` is `"sha256:" + lowercase(hex(SHA-256(UTF8("ForgeDock-Artifact-Payload-v1\0") || UTF8(JCS(payload)))))` for the closed, schema-validated payload that will be appended. The requested artifact MUST equal this profile, kind, ID, payload digest, sequence, and predecessor before signing, nonce consumption, append CAS, or publication. The capability, every delegated child, controller request, immutable operation record, and reconciliation request MUST preserve all six values. A different kind or payload under the same capability or `operationId` is a replay conflict, not a second append attempt.
...
- `lifecycle` is exactly `create` or `reuse`. `create` requires a `github.issue` subject and a registry entry with no allocated PR; `reuse` requires a `github.pr` subject whose repository and number exactly equal the PR already registered for `publicationId`. A controller MAY register a pre-existing PR only through the authenticated registry/marker transaction defined below: an already authenticated current marker MUST name the same publication, or the transaction MUST atomically prove the marker absent and install the first binding while claiming the physical target and source ref. A fresh read by itself is insufficient. A create result atomically completes the registry mapping with one allocated canonical PR subject; it never changes the capability subject.
...
A protected PR run marker is a closed object with exactly `profile`, `body`, and `proof`. `profile` is `forgedock.pr-run-marker/v1`. `body` has exactly `hostInstanceId`, `immutablePrRef`, `publicationId`, `runId`, `operationId`, `publicationVersion`, `sourceSha`, `issuedAt`, and `predecessorDigest`; the first marker has `predecessorDigest: null`, and a replacement names the digest of the prior complete marker. `proof` is exactly `{ "keyId": "...", "algorithm": "Ed25519", "signature": "<unpadded-base64url>" }`, where `keyId` is active for the configured host marker-signing service under the adapter/host trust root and:
...
The **authoritative publication registry** is durable semantic state in a configured controller/host trust boundary, never SQLite, a session, a workspace, or a rebuildable cache. Each registry record is a closed object with exactly `profile`, `body`, and `proof`; `profile` is `forgedock.publication-registry/v1`. Its `body` has exactly `registryId`, `registryVersion`, `previousRecordDigest`, `publicationId`, `runId`, `originSubject`, `lifecycle`, `headRepo`, `headRef`, `baseRepo`, `baseRef`, `buildResultId`, `buildResultDigest`, `sourceSha`, `publicationVersion`, `allocationState`, `prSubject`, `immutablePrRef`, `prState`, `currentRunMarker`, `sourceRefReservationId`, `targetReservationId`, `pendingPublication`, `tombstone`, and `committedAt`. `allocationState` is `unallocated` or `allocated`; the three PR members and `currentRunMarker` are `null` only while unallocated. `pendingPublication` is `null` or the closed value defined in §6.2. `tombstone` is `null` for a live record or exactly `{ "reason": "...", "tombstonedAt": "...", "releasedReservationIds": ["..."] }`; tombstoning never deletes history or lowers a high-water mark. No other nulls or fields are allowed.

Registry `proof` is exactly `{ "keyId": "...", "algorithm": "Ed25519", "signature": "<unpadded-base64url>" }`; its `keyId` MUST be active at `committedAt` under the configured controller registry trust root, and the controller verifies its lifecycle before accepting the signature over `UTF8("ForgeDock-Publication-Registry-Record-v1\0") || UTF8(profile) || 0x00 || UTF8(JCS(body))`. For the GitHub adapter, every record and high-water checkpoint MUST be durably anchored through authenticated host-protected metadata; if GitHub's native surface cannot supply it, a trusted adapter service is required and discovery reports the native surface unsupported. Ordinary issues, PR text, comments, labels, or local indexes are not registry storage. The store atomically CASes `(registryId, registryVersion, previousRecordDigest)`, enforces unique current `publicationId`, source-ref, immutable-PR, and complete head/base-tuple reservations, and retains signed registry/publication-version high-water marks, predecessor history, tombstones, and reservation release history across deletion, restart, restore, and failover.

The registry trust root and an independently retained authenticated high-water checkpoint MUST detect a restored/rolled-back record, missing tail, reset publication version, or resurrected reservation. Missing, unavailable, unverifiable, rolled-back, forked, or registry/host-marker-conflicting evidence denies capability issuance and every publication boundary. Recovery reconstructs from authenticated registry history plus protected host markers and committed receipts; cache-only mappings never authorize reuse. For a pre-existing PR with no marker, one registry transaction under the source-ref fence MUST freshly prove the exact PR is `OPEN`, prove the complete marker absent, uniquely reserve its immutable identity and complete tuple, and install a first authenticated marker before a later `reuse` capability is issued. That registration is an immutable controller operation bound to the selected verified BuildResult/current head, run, source ref, newly allocated publication version, and its own operation ID; the first `reuse` operation allocates a higher version and carries the registration marker as `expectedRunMarker`. If any claim exists or the transaction is indeterminate, registration fails closed. An authenticated existing marker may be adopted only by the publication identity already named by that marker.

The stable source-ref reservation and coordination domain is independent of `publicationId`, `runId`, issue/PR number, and base branch. Its canonical coordination-key object under [§8](#leases) has exactly `repository`, `resourceKind`, and `claim`: `repository` equals canonical `headRepo`, `resourceKind` is `pr-source-ref`, and the closed `claim` is exactly `{ "hostInstanceId": "...", "headRepo": "owner/name", "headRef": "refs/heads/..." }`. `constraints.coordinationKey` is that object's domain-separated hash. Before capability issuance the registry MUST atomically reserve this physical source ref for `publicationId`, or prove the existing reservation names that same identity. The reservation remains owned while its registered PR is `OPEN` and may be released only by an authenticated tombstone transaction after terminal-state confirmation and with no pending generation; history/high-water evidence remains. Every publication touching the ref, across all publication IDs and runs, MUST acquire this same key before workspace/source selection and retain it through receipt commit or the pending-generation recovery rules below. The remote boundary atomically verifies both the reservation and live fence before changing or selecting the ref. A ref reserved by another publication is a conflict before mutation; changing `baseRef` does not create another fencing domain.
...
| `artifactAppend` | Required only for `artifact.append`; all six members MUST equal the schema-validated artifact, delegated lineage, controller/host request, append CAS, and immutable operation record |
...
Unknown constraints, unavailable issuer or issuance records, absent replay state, clock uncertainty beyond configured skew, unsupported delegation, an unknown field, a missing exact subject, required revision, required `artifactAppend`, publication target, or source-ref coordination key, or a present forbidden revision, `artifactAppend`, or publication target are default-deny conditions. Capability possession never authorizes merge, publication, review, closure, or a transition without the typed controller's committed decision.
...
For `pr.publish`, discovery MUST explicitly advertise the complete fenced protocol below. The controller MUST derive `sourceSha`, `buildResultId`, and `buildResultDigest` only from one controller-selected, verified BuildResult for the same run/repository and MUST allocate `publicationVersion` by registry CAS. It then acquires the stable source-ref coordination key and freezes the operation record with the exact owner token and fencing sequence. Before the first external mutation, the retained workspace commit and selected local source ref MUST equal `sourceSha`; the authenticated registry record/digest, source-ref reservation, physical-target reservation, BuildResult, publication version, and prior marker MUST still equal the frozen request; and fresh host state MUST satisfy both target preconditions including `OPEN` for reuse. The controller then atomically changes the registry from ready to a pending publication generation. Any failure releases no authorization for mutation.

A pending-publication value is either `null` or a closed object with exactly `state`, `operationId`, `publicationVersion`, `publicationGeneration`, `registryVersion`, `registryRecordDigest`, `hostInstanceId`, `hostEpoch`, `discoveryChallenge`, `ownerTokenDigest`, and `fencingSequence`; `state` is `pending`, `receipt-ready`, `abort-ready`, or `recovery-required`. The registry and host durable operation state CAS the same generation. It remains authoritative across controller crash, lease expiry, and adapter restart.

The protocol has these linearization requirements:

1. **Remote source boundary.** A push MUST be a host-enforced compare-and-swap from the exact `remoteRefPrecondition` (including expected absence) to `sourceSha`. Selection without a push MUST atomically compare the ref to `sourceSha`. In either case the same call MUST validate the complete head/base tuple, authenticated registry record/digest, unique source-ref and physical-target reservations, stable source-ref coordination key, live owner token/fencing sequence, pending publication generation, operation ID, publication version, prior marker state, and BuildResult identity. The call MUST reject a reservation owned by another publication before changing the ref, even if that publication has another run, base, or `publicationId`. A preflight read followed by an unconditional push or branch selection is non-conforming.
2. **Create boundary.** `create` MUST atomically validate the issue subject, still-unallocated `publicationId`, exact head/base tuple, remote head `sourceSha`, expected marker absence, operation/fence/version/generation scope, and absence of conflicting source-ref, tuple, or immutable-PR claims before allocating one `OPEN` PR. In the same transaction it MUST reserve the returned immutable PR/tuple, append the complete authenticated run marker, CAS the current marker from absent to that marker, and return the canonical `github.pr` subject plus immutable reference. Discovery by head branch alone, choosing the first result, or accepting another base is forbidden.
3. **Reuse/update boundary.** `reuse` MUST address the exact registered repository/PR number and immutable reference, not discover by branch. Before any push or PR update, the host MUST atomically compare `prPrecondition.headSha`, `prPrecondition.prState`, the complete expected marker, registry record/digest, and reservations. At reuse/update it MUST atomically revalidate that exact PR identity, `OPEN` state, head/base tuple, remote and PR head `sourceSha`, operation/fence/version/generation scope, and complete prior marker, then append the new authenticated marker and compare-and-replace the protected current-marker pointer from the exact expected marker to the new marker in the same fenced transaction as the ref/PR update. A stale/copied marker, same-SHA PR with another number/base/publication identity, terminal PR, or duplicate reservation is a conflict with no mutation.
4. **Finalization boundary.** After create, reuse, or update, the adapter MUST perform a fresh atomic comparison of the exact returned PR identity/reference, `OPEN` state, tuple, complete current marker, registry reservations/version, remote head, and PR head against the frozen operation and pending generation. It then returns a closed `publicationReceipt` with exactly `profile`, `hostInstanceId`, `hostEpoch`, `discoveryChallenge`, `publicationGeneration`, `registryRecordDigest`, `operationId`, `publicationId`, `publicationVersion`, `buildResultId`, `buildResultDigest`, `prSubject`, `immutablePrRef`, `prState`, `headRepo`, `headRef`, `baseRepo`, `baseRef`, `sourceSha`, `runMarker`, `fencingSequence`, `observedAt`, and `proof`. `profile` is `forgedock.publication-receipt/v1`, `prState` is `OPEN`, and `proof` is exactly `{ "keyId": "...", "algorithm": "Ed25519", "signature": "<unpadded-base64url>" }`. The signature is:
...
The proof key MUST be active for the configured adapter/host trust root at `observedAt`. `hostInstanceId`, `hostEpoch`, and `discoveryChallenge` MUST equal the same fresh authenticated discovery used for this finalization; `publicationGeneration` MUST resolve to the durable host operation and exact pending registry record. The controller verifies the closed shape, JCS bytes, domain, algorithm, signer lifecycle, current host epoch/challenge, complete operation binding, and durable generation before receipt CAS. A receipt from an earlier epoch/challenge, an unknown/revoked key, or missing/inconsistent durable operation state is denied. During crash reconciliation the controller obtains fresh discovery and the host may re-attest the same durable completed generation with the new current challenge/epoch; a withheld old receipt alone can never be committed.
...
| Artifact append substitution | A valid `artifact.append` capability binds `BuildResult`, artifact ID `art_A`, payload digest `D1`, sequence 4, and predecessor `P`; the request/retry supplies `Outcome`, another ID, digest `D2`, sequence, or predecessor | Deny before signing, nonce consumption, append CAS, or host mutation; reconciliation may inspect only the original exact append operation |
...
| First PR creation | A `create` capability has an exact `github.issue` subject, unallocated `publicationId`, valid absent/current remote precondition, tuple, BuildResult/version, source `A`, and live source-ref fence | Atomically establish/select remote `A`, create exactly one `OPEN` PR for that tuple, install the complete authenticated marker, return and register its canonical PR subject/immutable reference, and commit only its fresh fenced receipt |
| Unmarked pre-existing PR registration | An exact unclaimed `OPEN` PR has no protected marker, and a controller registration operation binds its current head/verified BuildResult `A` | Under the shared source-ref fence, atomically prove marker absence, reserve the immutable PR/tuple/ref, and install the authenticated registration marker; only a later higher-version `reuse` capability may issue. Ordinary PR text cannot satisfy this vector |
| Exact PR reuse/update | A `reuse` capability names registered `OPEN` PR 18, its full tuple and exact prior marker for version 1/source `A` match, and version 2 validly publishes `B` | Under one source-ref fence, atomically compare the complete old marker/head/state, update the ref/PR, replace the marker with the version 2/`B` marker, and commit only the receipt bound to PR 18 and `B` |
...
| Different publications share one source ref | Valid operations for publication IDs P1/run R1/source `A` and P2/run R2/source `B` both name the same canonical `headRepo`/`headRef` (even with different bases) | Both resolve to one reservation/fencing domain. Only the uniquely reserved publication may mutate; P2 conflicts before ref CAS and cannot leave P1's PR pointing at `B` |
...
| Registry rollback or loss | A local cache presents an older publication version, or authenticated registry history/high-water evidence is unavailable, rolled back, forked, or conflicts with the host marker | Deny capability issuance and every mutation/reconciliation claim until authenticated recovery; do not recreate ownership from PR text or cache state |
...
| Lease expires with pending generation | The owner expires after a publication boundary but before receipt CAS, and a higher-sequence owner takes over | Expiry grants no mutation. The successor must reconcile and fresh-receipt commit, transfer/resume the exact generation, or prove atomic abort; otherwise `recovery-required` continues fencing the source ref/PR |
| Stale or ambiguous receipt | A receipt lacks the closed proof fields/domain, names a prior host epoch/challenge, uses an untrusted/revoked key, or has no matching durable publication generation | Deny receipt CAS and publication claim. Reconciliation requires current discovery and fresh re-attestation of the same durable generation; no new mutation |
