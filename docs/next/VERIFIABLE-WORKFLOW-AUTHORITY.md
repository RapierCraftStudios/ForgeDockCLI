...
```json
{
  ...
  "id": "art_01J...",
  "runId": "run_01J...",
  "subject": { "type": "github.pr", "hostAuthorityId": "gha_01j8m6f4x9k2q7v3c5n8r0t1wy", "repositoryId": "R_kgDOLfrozen123", "repo": "rapiercraftstudios/forgedockcli", "number": 18 },
...
```json
{
  ...
  "body": {
    ...
    "subject": { "type": "github.pr", "hostAuthorityId": "gha_01j8m6f4x9k2q7v3c5n8r0t1wy", "repositoryId": "R_kgDOLfrozen123", "repo": "owner/name", "number": 18 },
    "runId": "run_...",
...
```json
{
  ...
  "audience": "aud_...",
  "action": "pr.publish",
  "subject": { "type": "github.issue", "hostAuthorityId": "gha_01j8m6f4x9k2q7v3c5n8r0t1wy", "repositoryId": "R_kgDOLfrozen123", "repo": "owner/name", "number": 90 },
  "runId": "run_...",
  "sourceSha": "89abcdef0123456789abcdef0123456789abcdef",
  "publicationTarget": {
    "publicationId": "pub_...",
    "lifecycle": "create",
    "originSubject": { "type": "github.issue", "hostAuthorityId": "gha_01j8m6f4x9k2q7v3c5n8r0t1wy", "repositoryId": "R_kgDOLfrozen123", "repo": "owner/name", "number": 90 },
    "registryRunId": "run_...",
    "hostAuthorityId": "gha_01j8m6f4x9k2q7v3c5n8r0t1wy",
    "repositoryId": "R_kgDOLfrozen123",
    "sourceRefPolicy": {
...
`publicationTarget` is required only for `pr.publish` and forbidden for every other action. It is a closed object with exactly `publicationId`, `lifecycle`, `originSubject`, `registryRunId`, `hostAuthorityId`, `repositoryId`, `sourceRefPolicy`, `headRepo`, `headRef`, `baseRepo`, `baseRef`, `buildResultId`, `buildResultDigest`, `publicationVersion`, `remoteRefPrecondition`, and `prPrecondition`:

- `publicationId` is a controller-generated opaque identity whose authoritative registry entry is immutably bound to the canonical subject's stable `(hostAuthorityId, repositoryId)`, signed `repo` metadata, run, originating issue or pre-existing PR, signed source-ref policy, head repository/ref, and base repository/ref. This identity remains stable when a create operation allocates a PR number.
...
- `hostAuthorityId` and `repositoryId` are copied from `originSubject` and fresh authenticated discovery. They MUST equal the capability subject's immutable repository pair and the registry, checkpoint, reservation, dedicated source-ref key, operation, request, marker-bound subject, immutable PR reference, and receipt bindings. The configured authority registry MUST enforce one globally shared authority/lease namespace for each authenticated `(endpoint, trustRoot, repositoryId)` and MUST reject configuration, registration, or issuance if the same physical endpoint/repository is assigned multiple authority IDs or repository IDs. Neither canonical display metadata nor ephemeral `hostInstanceId` substitutes for either immutable value.
...
For every reusable host mutation, the reusable `operationId` MUST be the exact host idempotency/deduplication key and the operation-identity key used for durable host retry and reconciliation lookup; it MUST NOT be replaced by or collapsed to a capability/nonce-derived or other shared key. The controller MUST send that exact key with the `logicalRequestDigest` and complete frozen request scope. On first acceptance, the host or conforming adapter MUST atomically bind the key to that digest and scope. An exact duplicate MUST return or reconcile the same stored or in-flight operation without crossing the host mutation boundary again, while reuse of the key with a different digest or scope MUST be rejected before mutation.
...
A subject is a closed typed object, not a URL or display name. The four and only four canonical subject forms in this profile are:

```json
{ "type": "github.repo", "hostAuthorityId": "gha_01j8m6f4x9k2q7v3c5n8r0t1wy", "repositoryId": "R_kgDOLfrozen123", "repo": "owner/name" }
{ "type": "github.issue", "hostAuthorityId": "gha_01j8m6f4x9k2q7v3c5n8r0t1wy", "repositoryId": "R_kgDOLfrozen123", "repo": "owner/name", "number": 90 }
{ "type": "github.pr", "hostAuthorityId": "gha_01j8m6f4x9k2q7v3c5n8r0t1wy", "repositoryId": "R_kgDOLfrozen123", "repo": "owner/name", "number": 18 }
{ "type": "git.commit", "hostAuthorityId": "gha_01j8m6f4x9k2q7v3c5n8r0t1wy", "repositoryId": "R_kgDOLfrozen123", "repo": "owner/name", "sha": "0123456789abcdef0123456789abcdef01234567" }
```

Each form has exactly the displayed members; missing, `null`, or unknown members are rejected. `type` is one exact lower-case value above. `hostAuthorityId` is the stable configured host authority from authenticated discovery, and `repositoryId` is the host's immutable repository resource identifier in that authority's namespace. Each is an ASCII string of 1..128 characters from `[A-Za-z0-9._:-]`, is compared byte-for-byte, and MUST be obtained through the independently trusted adapter configuration and authenticated host API rather than repository text or a caller claim. `repo` is the host-confirmed canonical lower-case `owner/name`, with exactly one `/`, each component 1..100 characters, and characters from `[a-z0-9._-]`; URLs, clone URLs, aliases, titles, and shorthand are forbidden. `number` is a positive safe integer in `1..9007199254740991`. `sha` is exactly 40 lower-case hexadecimal characters.

`(hostAuthorityId, repositoryId)` is the immutable repository identity. Child identity additionally includes `type` and `number`, or `type` and `sha`; a repository subject has no child discriminator. `repo` is signed canonical display metadata and authenticated historical context, but is not an identity key. Consequently a rename changes the canonical bytes of a newly issued subject without changing which repository it denotes; historical subjects remain attributable by the immutable pair and preserve the canonical name observed when signed. A transfer within the same authority has the same behavior only when the host guarantees that `repositoryId` survives transfer. A transfer that changes authority or immutable ID is a different repository. The same `repo` under another authority or ID, an alias with the same ID presented where current canonical metadata is required, and issue/PR number equality across distinct immutable pairs are never equal.

Exact **subject equality** means equality of every closed member and is required whenever this contract says a capability, protected artifact, controller request, operation, host request/result, event, registry subject, marker-bound subject, or bundle root subject “equals” another subject. **Resource identity equality** ignores only `repo` and is used solely for historical rename/transfer attribution after an authenticated host mapping proves the old and current canonical names belong to the same immutable pair; it never authorizes replay of old capability or request bytes. Capability issuance and protected operations MUST use the fresh canonical `repo`, preserve the complete subject through delegation and every boundary, and reject name-only, URL-derived, authority-substituted, or repository-ID-substituted subjects. Canonical subject bytes are UTF-8 JCS under [§3.2](#canonical-bytes) and are signed exactly.

Repository-valued fields outside a canonical subject, including `headRepo`, `baseRepo`, and coordination-key `repository`, remain canonical display-name metadata only where their containing closed schema says so. Their containing capability, registry, discovery, or receipt MUST also bind the same `hostAuthorityId` and immutable `repositoryId` through its canonical subject or repository binding; a name alone never establishes repository authority.

### 6.2 Discovery and conformance

Every host adapter MUST expose the following closed response. Unknown members at the envelope, `body`, `repository`, any guarantee object, or `proof` level are rejected recursively; duplicate JSON names are malformed.

```json
{
  "profile": "forgedock.host-discovery/v1",
  "body": {
    "adapterId": "github-protected",
    "adapterVersion": "1.0.0",
    "hostAuthorityId": "gha_01j8m6f4x9k2q7v3c5n8r0t1wy",
    "hostInstanceId": "instance-7",
    "endpoint": "https://api.github.example/v3",
    "repository": { "type": "github.repo", "hostAuthorityId": "gha_01j8m6f4x9k2q7v3c5n8r0t1wy", "repositoryId": "R_kgDOLfrozen123", "repo": "owner/name" },
    "supportedSubjectTypes": ["git.commit", "github.issue", "github.pr", "github.repo"],
    "supportedOperations": ["artifact.append", "artifact.read"],
    "guarantees": {
      "lookup": { "exactFullSha": true, "canonicalSubjects": true },
      "limits": { "maxCommentBytes": 65536, "maxArtifactBytes": 1048576 },
      "events": { "authenticated": true, "ordered": true, "replayable": true },
      "coordination": { "compareAndSwap": true, "leases": true, "fencing": true },
      "publication": { "protectedArtifacts": true, "protectedPullRequests": false, "fencedReceipts": false }
    },
    "issuedAt": "2026-08-03T16:01:12Z",
    "validFrom": "2026-08-03T16:01:12Z",
    "expiresAt": "2026-08-03T16:06:12Z",
    "epoch": 42,
...
The top level has exactly `profile`, `body`, and `proof`; `profile` is exactly `forgedock.host-discovery/v1`. `body` has exactly the 14 displayed members. `adapterId`, `adapterVersion`, and `hostInstanceId` are non-empty printable ASCII strings of at most 128 bytes; they are opaque and compared byte-for-byte. `hostAuthorityId` follows §6.1 and MUST equal `repository.hostAuthorityId`. `repository` is the exact §6.1 repository subject. `endpoint` is an independently configured absolute HTTPS URI of at most 2048 ASCII bytes: scheme and DNS host are lower case, international host names are A-labels, user information/query/fragment are absent, the default port is omitted, percent escapes use upper-case hexadecimal, path dot segments and repeated slashes are removed, and `/` is the only permitted trailing slash. The verifier performs this normalization before configuration and requires the response to equal that canonical value; redirects or aliases do not establish equality.

`supportedSubjectTypes` and `supportedOperations` are non-empty, duplicate-free arrays in ascending UTF-8 byte order, with at most 32 entries. Subject entries are from the four §6.1 types. Operation entries are from §5's closed vocabulary. Arrays are signed in their displayed order and are never sorted by a verifier after receipt. `guarantees` has exactly `lookup`, `limits`, `events`, `coordination`, and `publication`, each with exactly the displayed members. Every guarantee is a JSON boolean except the two limits. Limits are safe integers in `0..67108864`; zero means the corresponding operation is unsupported, not unlimited. A `true` claim advertises the complete applicable guarantee in this document, not a weaker best effort. Inconsistent claims are malformed: `fencing` requires `compareAndSwap` and `leases`; `fencedReceipts` requires `protectedPullRequests`; and an advertised protected operation requires its corresponding operation string.

Discovery times use RFC 3339 UTC seconds and `Z` exactly, with no fraction. They satisfy `validFrom <= issuedAt < expiresAt` and `expiresAt - issuedAt <= 600 seconds`. `epoch` is a positive safe integer and is allocated by an authority-scoped durable monotonic counter; it never resets across adapter instance, restart, restore, or failover. `challenge` is unpadded base64url encoding of exactly 32 random bytes freshly generated by the controller for the expected operation class. The controller stores each challenge as outstanding for one response only; equality is byte-for-byte after strict decoding, and reuse is replay even if all other fields are current.

The proof has exactly `keyId`, `algorithm`, and `signature`. `algorithm` is exactly `Ed25519`; `signature` is strict unpadded base64url of exactly 64 bytes. Discovery signing public keys are raw 32-byte Ed25519 keys, reject non-canonical or small-order encodings, and use the independently checkable identifier:

```text
discoveryKeyId = "hdk_" + lowercase(hex(publicKeyBytes))
```

No truncation, alternate encoding, or claimed key ID is equivalent. Define the exact signed bytes:

```text
bodyBytes = UTF8(JCS(body))
discoverySignatureInput =
  UTF8("ForgeDock-Host-Discovery-Signature-v1\0") ||
  UTF8(profile) || 0x00 || bodyBytes
```

The Ed25519 proof is over `discoverySignatureInput`. This binds the profile and complete closed body; whitespace, member order, an altered guarantee, and any identity, endpoint, time, epoch, or challenge change require a different signature. Ordinary JSON serialization is not evidence of the signed bytes.

Before sending the challenge, the controller MUST select one independently configured mapping `(canonical endpoint, hostAuthorityId, repositoryId) -> (allowed adapter IDs, discovery trust root, lifecycle source, maximum age)`. Neither `hostAuthorityId`, `repository`, `keyId`, public key, trust-root ID, lifecycle record, endpoint alias, nor checkpoint carried in the response may select or extend this mapping. The verifier obtains the proof key and authenticated lifecycle from that external trust root, requires `keyId` to equal the derived ID, and requires the signer to have been active and authorized for this authority at `issuedAt` and to remain currently authorized under policy. Unknown or unavailable trust, an attacker-supplied key or lifecycle record, mismatched authority, a signer outside its activation/expiry interval, retirement or revocation effective at the applicable time, or current revocation when current authorization is required fails closed as `unverifiable` or `revoked` under §3.4 precedence; it never bootstraps trust from discovery.

Verification performs, in order: closed JSON/JCS/type/bound checks; independent trust-map selection; key derivation and lifecycle checks; proof decoding/signature verification; expected adapter/authority/endpoint/immutable repository and current canonical `repo` binding; exact outstanding challenge consumption; trusted-time checks `validFrom <= now < expiresAt`, `issuedAt <= now`, and `now - issuedAt <= min(configured maximum age, 600 seconds)`; then comparison and atomic retention of the authority-scoped highest accepted epoch and response digest. The epoch MUST be strictly greater than the retained value for a new probe. An exact duplicate is replay, not idempotent success. A lower/equal epoch, removed operation/guarantee relative to the controller-required baseline, changed authority mapping, stale challenge, or unavailable rollback/replay store is downgrade, replay, or `unverifiable`. The controller relies only on guarantees after all checks pass and only for the operation class tied to that challenge. Unrelated successful API calls and ephemeral `hostInstanceId` never substitute for discovery.
...
| Discovery vector | Mutation/precondition | Required result before guarantee use |
| --- | --- | --- |
| Signed field | `maxArtifactBytes` becomes `1048575` | `invalid-signature` |
| Unknown field | Add `body.debug: true`, `guarantees.lookup.aliases: true`, or `proof.publicKey` | `malformed`; unknown fields are checked before proof acceptance |
| Challenge | Expected challenge is any value other than 32 zero bytes, or the fixture challenge was already consumed | `wrong-binding` or replay diagnostic; no epoch update |
| Authority/repository | Configure another `hostAuthorityId` or `repositoryId`, including the same `repo` | `wrong-binding`; response values cannot select another trust mapping |
| Endpoint | Configure `https://api.github.example/v4` or present a redirect/alias | `wrong-binding` |
| Epoch rollback | Retained authority epoch is 42 or 43 | `unverifiable` rollback/replay; lower/equal epoch is never accepted |
| Time bounds | Trusted `now` is before `validFrom`, at/after `expiresAt`, or more than configured maximum age after `issuedAt`; alternatively make the signed interval exceed 600 seconds and re-sign | `expired`, or `malformed` for the excessive interval |
| Proof encoding | Add `=` padding, use non-url base64, decode to 63/65 bytes, or corrupt one signature byte | `malformed` for encoding/length; otherwise `invalid-signature` |
| Trust/lifecycle | Remove the external mapping, supply the fixture key only in the response, map it to another authority, or mark it unavailable/revoked/retired for the applicable time | `unverifiable` or `revoked` under §3.4; never `verified` |
| Downgrade | Re-sign after removing a required operation or changing a controller-required guarantee to `false` | Valid cryptography but required baseline mismatch; `unverifiable` and no protected operation |

An adapter conformance suite MUST test the fixed positive discovery value/signature output and every reject mutation above; canonical subject round trips and immutable authority/repository substitutions; repository ownership and rename behavior; exact full-SHA lookup, stale-head detection, idempotency keys, response-to-request binding, pagination/completeness, error classification, size limits, signed challenge discovery, expiry, replay, epoch rollback/downgrade, and authenticated host-authority binding while treating host instance only as freshness metadata. For PR publication it additionally MUST test exact issue/PR lifecycle identity and `OPEN` state, full head/base tuple selection, immutable returned PR references, authenticated marker storage/replacement, durable registry and rollback evidence, source-ref/physical-target uniqueness, pending-generation takeover, required fencing/CAS, atomic source-SHA preconditions, and receipt signer/domain/challenge/epoch verification. It MUST distinguish permission denied, not found, conflict, rate limit, stale revision, unsupported guarantee, and indeterminate network result. An indeterminate mutation result is reconciled by its original idempotency key, exact subject, complete publication target when applicable, and action-specific revision binding before retry; the controller MUST NOT blindly duplicate a side effect.
...
| --- | --- | --- |
| Immutable subject accepted | Artifact, capability, discovery repository, and request all carry `{type:"github.issue", hostAuthorityId:H1, repositoryId:R1, repo:"owner/name", number:90}` in their closed canonical locations | Subject binding succeeds; canonical bytes include all five members and authority may proceed to the remaining checks |
| Same name, different authority | A request preserves `repo:"owner/name"`, `repositoryId:R1`, type, and child discriminator but substitutes `hostAuthorityId:H2` | Not equal by resource identity or exact subject equality; deny as `wrong-binding` before capability use or host mutation |
| Same name, different repository ID | A request preserves `repo:"owner/name"`, `hostAuthorityId:H1`, type, and child discriminator but substitutes `repositoryId:R2` | Not equal; deny as `wrong-binding`; host-confirmed display-name equality contributes no authority |
| Canonical rename | Historical evidence is signed as `(H1,R1,"old-owner/old-name")`; authenticated current host mapping returns `(H1,R1,"new-owner/new-name")` | Historical attribution may link the subjects by immutable resource identity while preserving both signed byte strings. New capability/request bytes MUST use the new canonical metadata; old bytes are not exactly equal and cannot authorize replay |
| Transfer changes immutable pair | Canonical display metadata remains or becomes `owner/name`, but transfer changes `hostAuthorityId` or `repositoryId` | Treat as a different repository; aliases, prior capabilities, checkpoints, and name equality cannot bridge the identity |
| Name-only subject rejected | Any canonical subject omits `hostAuthorityId` or `repositoryId`, or a nested subject adds an unknown member | `incomplete` for a missing required member or `malformed` for the closed-shape violation; no authority |
...
| Two reusable logical invocations | One currently valid `oneShot: false` capability is used for two complete requests with fresh controller-issued logical request IDs `L1` and `L2` | The reusable operation formula produces distinct operation IDs; for host mutations, those IDs are the distinct exact host idempotency keys. Each request/digest is independently recorded and, if both complete, returns a distinct operation-bound result record rather than aliasing either invocation to the other |
| Exact reusable replay | A retry presents the same `capabilityId`, `logicalRequestId`, operation ID, logical request digest, complete frozen request bytes, and currently authenticated lineage/presenter as the first invocation | Perform no second side effect; for a host mutation, the controller and host use that same operation ID as the exact host idempotency key and do not cross the host mutation boundary again, but only return or reconcile the original stored or in-flight operation/result |
...
`repository` and `claim.headRepo` are the same canonical display name. `claim` has exactly `hostAuthorityId`, `repositoryId`, `headRepo`, and `headRef`; the immutable pair equals the canonical capability subject, publication target, and fresh discovery repository. `hostAuthorityId` is stable across adapter processes, instances, epochs, restarts, and failover, and `repositoryId` is stable across repository rename/transfer where the host preserves resource identity; ephemeral discovery `hostInstanceId`, subject child discriminator, run, issue/PR, publication ID, base, and owner are forbidden. Its exact hash is:
...
Offline verification starts with a preconfigured trust root or an independently authenticated controller checkpoint supplied outside the bundle. The verifier selects that anchor before parsing any bundle claim that could influence trust selection and records its out-of-band source and covered trust-root/checkpoint digest in the verification report. Bundle-carried `trustRootId`, public keys, lifecycle records, checkpoints, and checkpoint references are evidence only: they may extend a chain from the selected anchor, but MUST NOT select, replace, or bootstrap it. A checkpoint copied from the bundle into the verifier's “anchor” input in the same trust transaction is still bundle-contained and untrusted. Without an independently authenticated anchor, the result is `unverifiable` (or a separate cryptographically-valid-but-untrusted diagnostic), never `verified`. Verification checks the exact archive set, limits, member lengths/digests, manifest and bundle signature, trusted key lifecycle, artifact canonical bytes, signatures, immutable subject bindings, chain/checkpoint evidence, and legacy status without network access. Missing trust metadata, missing predecessor members, stale/revoked keys, expired discovery, or an absent anchor are reported rather than repaired. Offline verification cannot establish current host state, current merge policy, current repository contents, or current revocation status beyond the independently anchored trust snapshot.

Focused bundle trust vector: given an otherwise byte-perfect bundle whose signatures, included keys, lifecycle chain, and included checkpoint all verify mathematically, configure no trust root and supply no independently authenticated checkpoint. The required primary result is `unverifiable`, with an optional cryptographically-valid-but-untrusted diagnostic, and no member or chain is `verified`. Repeating the case while passing the bundle's own checkpoint or `trustRootId` as the purported anchor has the same result. A positive case first configures an out-of-bundle root/checkpoint digest and may report `verified` only for the artifact/key/checkpoint chain transitively covered by that exact anchor; evidence before, beside, or after the covered chain remains separately untrusted or unverifiable.
...
For run `run_demo`, the controller emits a genesis `Investigation` at sequence 0 with `predecessor: null` and canonical subject `{ "type": "github.issue", "hostAuthorityId": "gha_01j8m6f4x9k2q7v3c5n8r0t1wy", "repositoryId": "R_kgDOLfrozen123", "repo": "owner/name", "number": 90 }`. It then emits a `BuildResult` at sequence 1 whose predecessor is the exact digest of sequence 0. A `ReviewVerdict` at sequence 2 names the exact PR subject `{ "type": "github.pr", "hostAuthorityId": "gha_01j8m6f4x9k2q7v3c5n8r0t1wy", "repositoryId": "R_kgDOLfrozen123", "repo": "owner/name", "number": 18 }` and full head SHA in both its payload and signature input. The controller verifies canonical bytes, each digest, the trusted active key, sequence links, exact subject/run/SHA, and current host head before recording the verdict. A later merge capability additionally binds the exact MergeRequest and that ReviewVerdict IDs/digests and proves the approval remains the current non-superseded decision at the merge boundary.
...
Before opening the portable export, the offline verifier is independently configured with trust root `root_demo_2026` and its authenticated public-key/checkpoint digest obtained from operator-controlled configuration outside the archive. The export contains the manifest, the three exact artifact members, the public key and lifecycle record, and a checkpoint naming sequence 2/digest. Bundle-carried `trustRootId: root_demo_2026`, keys, lifecycle record, and checkpoint are evidence, not the source of trust. The verifier matches and extends only the chain covered by the externally supplied root/checkpoint, so offline verification can establish the signed chain through sequence 2 and no later or unrelated chain. If that external anchor is omitted—or if the bundle's own checkpoint is passed back as its replacement—the same mathematically valid export is `unverifiable`, never `verified`. Import remains archival, and a live controller must re-check the current PR head and policy before merge.
